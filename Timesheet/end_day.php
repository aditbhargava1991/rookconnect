...
if($staff > 0) {
	...
	foreach ($all_contacts as $staff) {
		...
		if(!empty($shifts) && $timesheet_track_shifts == '1' && $tracking_id['id'] > 0) {
			...
			mysqli_query($dbc, "UPDATE `time_cards` SET `timer_tracked` = (($time - `timer_start`) + IFNULL(`timer_tracked`,0)) / 3600, `timer_start`=0, `type_of_time`=IF(`day_tracking_type` IS NULL OR `day_tracking_type` = '', 'Regular Hrs.', `day_tracking_type`), `end_time`='$hour', `end_time_from` = 'End Day', `comment_box`=CONCAT(IFNULL(CONCAT(`comment_box`,'&lt;br /&gt;'),''),'$comment') $highlight_query WHERE `timer_start` > 0 AND `type_of_time`='day_tracking' AND `staff`='$staff' $id_query");
