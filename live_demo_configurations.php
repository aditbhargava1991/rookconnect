...
<div id="no-more-tables">
	...
	<?php if(!DATABASE_NAME2) {
		...
		<table class="table table-bordered">
			...
			<?php $field_config_tables = mysqli_fetch_all(mysqli_query($dbc, "SHOW TABLES WHERE `Tables_in_".DATABASE_NAME."` LIKE '%_config%'")); ?>
			<?php foreach($field_config_tables as $field_config_table) { ?>
					<?php $table_name = $field_config_table[0]; ?>
					<?php if($table_name == 'field_mandatory_config') continue; ?>
					<?php $db_all->query("ALTER TABLE `".DATABASE_NAME2."`.`$table_name` ADD `deleted` TINYINT(1) DEFAULT 0"); ?>
					<?php $db_all->query("ALTER TABLE `".DATABASE_NAME."`.`$table_name` ADD `deleted` TINYINT(1) DEFAULT 0"); ?>
					<tr data-table="<?php echo $table_name; ?>">
						<?php
							$count_live = $db_all->query("SELECT SUM(IF(`deleted`=0,1,0)) `active`, SUM(IF(`deleted`=0,0,1)) `archived` FROM `".DATABASE_NAME2."`.`$table_name`")->fetch_assoc();
							$count_demo = $db_all->query("SELECT SUM(IF(`deleted`=0,1,0)) `active`, SUM(IF(`deleted`=0,0,1)) `archived` FROM `".DATABASE_NAME."`.`$table_name`")->fetch_assoc();
							$cols = $db_all->query("SELECT SUM(IF(`table_schema`='".DATABASE_NAME2."',1,0)) `live_cols`, SUM(IF(`table_schema`='".DATABASE_NAME."',1,0)) `demo_cols` FROM `information_schema`.`columns` WHERE `table_name`='$table_name'")->fetch_assoc();
						?>
						<td data-title="Table Name"><?= $table_name ?></td>
						<td data-title="Rows in Live Software"><?= $count_live['active'] ?> Fields, <?= $count_live['archived'] ?> Archived</td>
						<td data-title="Rows in Demo Software"><?= $count_demo['active'] ?> Fields, <?= $count_demo['archived'] ?> Archived</td>
