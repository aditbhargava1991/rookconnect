...
foreach($equip_list as $equipment) {
	...
	if(get_config($dbc, 'dispatch_tile_hide_empty') != 1 || !empty($tickets)) {
		...
		$equipment_buttons[] = '<a href="" data-region=\''.json_encode($equip_regions).'\' data-location=\''.json_encode($equip_locations).'\' data-classification=\''.json_encode($equip_classifications).'\' data-equipment="'.$equipment['equipmentid'].'" data-activevalue="'.$equipment['equipmentid'].'" onclick="select_equipment(this); return false;"><div class="pull-left" style="background-color: #'.$title_color.'; width: 20px; height: 20px; margin: 5px;"></div><li data-equipment="'.$equipment['equipmentid'].'" class="dispatch-equipment-button '.(($reset_active != 1 && (empty($active_equipment) || in_array($equipment['equipmentid'], $active_equipment)) || 1==1) || ($reset_active == 1 && !empty($tickets)) ? 'active blue' : '').'">'.$equipment['label'].'</li></a><div class="clearfix"></div>';
