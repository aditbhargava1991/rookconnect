...
function toggle_columns(type = '', reload_teams = 0) {
	...
	if(teams.length > 0 || all_contacts.length > 0 || clients.length > 0) {
	...
	<?php if($_GET['type'] != 'my') { ?>
		...
		$.ajax({
			...
			method: 'POST',
			data: { staff: visibles, teams: teams, clients: clients, region: regions },
					...
					<div id="collapse_staff" class="panel-collapse collapse in">
						<div class="panel-body panel-body-height" style="overflow-y: auto; padding: 0;">
							...
							foreach($staff_list as $staff_id) {
								echo "<a href='' onclick='$(\"#collapse_teams .block-item\").removeClass(\"active\"); $(this).find(\".block-item\").toggleClass(\"active\"); toggle_columns(); return false;'><div class='block-item ".(in_array($staff_id,$active_staff) ? 'active' : '')."' data-staff='$staff_id' data-category='".get_contact($dbc, $staff_id, 'category_contact')."' data-region='".get_contact($dbc, $staff_id, 'region')."' data-activevalue='".$staff_id."'>";
