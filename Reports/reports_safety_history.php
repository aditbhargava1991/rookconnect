...
if (isset($_POST['printpdf'])) {
	...
	//$pdf->writeHTML($html, true, false, true, false, '');
	$pdf->Output('Download/safety_history_'.$today_date.'.pdf', 'F');
    track_download($dbc, 'history_safety_history', 0, WEBSITE_URL.'/Reports/Download/safety_history_'.$today_date.'.pdf', 'Safety History Report');

    ?>

	<script type="text/javascript" language="Javascript">
	window.open('Download/safety_history_<?php echo $today_date;?>.pdf', 'fullscreen=yes');
	</script>
    <?php
...
function report_receivables($dbc, $starttime, $endtime, $table_style, $table_row_style, $grand_total_style) {
	...
	$query = mysqli_fetch_all(mysqli_query($dbc, "SELECT * FROM safety_history where updated_at >= '$starttime' and updated_at < '$endtime' ORDER BY updated_at DESC"));
    $odd_even = 0;
...
function AddPlayTime($times) {
    // loop tsafetyought all the times
    foreach ($times as $time) {
