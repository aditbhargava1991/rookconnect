...
if (isset($_POST['printpdf'])) {
	...
	class MYPDF extends TCPDF {
		...
		public function Footer() {
			...
			// DB Backupition at 15 mm from bottom
			$this->SetY(-15);
            $this->SetFont('helvetica', 'I', 9);
	...
	//$pdf->writeHTML($html, true, false, true, false, '');
	$pdf->Output('Download/db_backup_history_'.$today_date.'.pdf', 'F');
    track_download($dbc, 'history_db_backup_history', 0, WEBSITE_URL.'/Reports/Download/db_backup_history_'.$today_date.'.pdf', 'Point of Sale History Report');

    ?>

	<script type="text/javascript" language="Javascript">
	window.open('Download/db_backup_history_<?php echo $today_date;?>.pdf', 'fullscreen=yes');
	</script>
    <?php
...
function report_receivables($dbc, $starttime, $endtime, $table_style, $table_row_style, $grand_total_style) {
	...
	$query = mysqli_fetch_all(mysqli_query($dbc, "SELECT * FROM db_backup_history where updated_at >= '$starttime' and updated_at < '$endtime' ORDER BY updated_at DESC"));
    $odd_even = 0;
...
function AddPlayTime($times) {
    // loop tdb_backupought all the times
    foreach ($times as $time) {
