...
<ul class="sidebar">
    ...
    <?php } ?>
	<?php if(in_array('Admin',$tile_tabs) && $approvals > 0) {
		if($admin_access) { ?>
			<li class="sidebar-higher-level highest-level"><a class="cursor-hand <?= $current_file == 'admin.php' && $_GET['manager_approvals'] != 1 ? 'active blue' : 'collapsed' ?>" data-toggle="collapse" data-target="#admin_stat">Administration<span class="arrow pull-right"></span></a>
				<ul id="admin_stat" class="collapse <?= $current_file == 'admin.php' && $_GET['manager_approvals'] != 1 ? 'in' : '' ?>">
		...
		<?php }
		if($manager_access) { ?>
			<li class="sidebar-higher-level highest-level"><a class="cursor-hand <?= $current_file == 'admin.php' && $_GET['manager_approvals'] == 1 ? 'active blue' : 'collapsed' ?>" data-toggle="collapse" data-target="#manager_stat"><?= $manager_approvals_tab ?><span class="arrow pull-right"></span></a>
				<ul id="manager_stat" class="collapse <?= $current_file == 'admin.php' && $_GET['manager_approvals'] == 1 ? 'in' : '' ?>">
					<?php foreach (str_getcsv(html_entity_decode($get_field_config['incident_types']), ',') as $in_type) { ?>
						<?php $counts = $dbc->query("SELECT SUM(IF(`manager_status`='Pending' OR IFNULL(`manager_status`,'')='',1,0)) `pending`, SUM(IF(`manager_status`='Review',1,0)) `review`, SUM(IF(`manager_status`='Revision',1,0)) `revision`, SUM(IF(`manager_status`='Approved',1,0)) `approved` FROM `incident_report` WHERE `deleted`=0 AND `type`='$in_type'")->fetch_assoc(); ?>
						<li class="sidebar-higher-level"><a class="cursor-hand <?= $in_type == $current_type ? 'active blue' : 'collapsed' ?>" data-toggle="collapse" data-target="#<?= config_safe_str($in_type) ?>"><?= $in_type ?><span class="arrow pull-right"></span></a>
							<ul id="<?= config_safe_str($in_type) ?>" class="collapse <?= $current_type == $in_type ? 'in' : '' ?>">
								<a href="admin.php?status=Pending&type=<?= $in_type ?>"><li <?= $current_file == 'admin.php' && $in_type == $current_type && $_GET['status'] == 'Pending' ? 'class="active"' : '' ?>>Pending<span class="pull-right"><?= $counts['pending'] > 0 ? $counts['pending'] : 0 ?></span></li></a>
								<a href="admin.php?status=Revision&type=<?= $in_type ?>"><li <?= $current_file == 'admin.php' && $in_type == $current_type && $_GET['status'] == 'Revision' ? 'class="active"' : '' ?>>In Revision<span class="pull-right"><?= $counts['revision'] > 0 ? $counts['revision'] : 0 ?></span></li></a>
								<a href="admin.php?status=Review&type=<?= $in_type ?>"><li <?= $current_file == 'admin.php' && $in_type == $current_type && $_GET['status'] == 'Review' ? 'class="active"' : '' ?>>In Review<span class="pull-right"><?= $counts['review'] > 0 ? $counts['review'] : 0 ?></span></li></a>
								<a href="admin.php?status=Approved&type=<?= $in_type ?>"><li <?= $current_file == 'admin.php' && $in_type == $current_type && $_GET['status'] == 'Approved' ? 'class="active"' : '' ?>>Done<span class="pull-right"><?= $counts['done'] > 0 ? $counts['done'] : 0 ?></span></li></a>
							</ul>
						</li>
					<?php } ?>
				</ul>
