...
<?php if($communication_type == 'External') { ?>
    ...
    foreach($tags as $tag_contact) { ?>
        <div class="form-group multi-block" data-type="communication_tags">
            ...
            <div class="col-sm-1">
                <img class="pull-right inline-img cursor-hand no-toggle" src="../img/remove.png" title="Remove <?= CONTACTS_NOUN ?>" onclick="remMulti(this);">
                <img class="pull-right inline-img cursor-hand no-toggle" src="../img/icons/ROOK-add-icon.png" title="Add <?= CONTACTS_NOUN ?>" onclick="addMulti(this);">
            </div>
        </div>
    <?php }
} ?>