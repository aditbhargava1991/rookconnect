				...
				<?php if($field_sort_field == 'Staff Check In' && strpos($value_config,',Staff Check In,') !== FALSE) { ?>
					<div class="col-sm-2" style="<?= strpos($value_config,',Staff Check In,') === FALSE ? 'display: none;' : '' ?>">
						<div class="<?= $access_staff_checkin == TRUE || $staff['item_id'] == $_SESSION['contactid'] ? 'toggleSwitch staffSwitch mobile-lg' : '' ?>" style="margin-top: -1em;">
							...
							<span style="<?= $staff['arrived'] > 0 && !empty($staff['timer_start']) ? 'display: none;' : '' ?>"><img src="<?= WEBSITE_URL ?>/img/icons/switch-6.png" class="text-lg inline-img on_img"> Not Checked In</span>
							<span style="<?= $staff['arrived'] > 0 && !empty($staff['timer_start']) ? '' : 'display: none;' ?>"><img src="<?= WEBSITE_URL ?>/img/icons/switch-7.png" class="text-lg inline-img off_img"> Checked In</span>
						</div>
					</div>
				<?php } ?>
			<?php } ?>
